...
<template>
  <ContentLayout :no-padding="true" classes="bg-light">
    <IonCard class="ion-padding" style="margin: 0;">
      <div class="section">
        <div class="flex">
        ...
        <div class="info-row">
          <div class="bg-lightprimary">
            <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
                ...
                fill="#71438D"
              />
            </svg> Escolas <strong>{{ alertasInformacoes.escolas }}</strong>
          </div>
          <div class="bg-lighttertiary">
            ...
            Turmas: <strong>{{ alertasInformacoes.turmas }}</strong>
          </div>
        </div>
      </div>
      <div class="section">
        ...
        <div class="grid">
          <div class="row bg">
            <div
              v-for="dia in quadroHorarios.dias" :key="dia.value" :class="{ active: dia.value === selectDay }"
      ...
      <div class="section">
        ...
        <div class="bg-primary ion-padding">
          <Swiper :slides-per-view="1.2" :pagination="true" space-between="10" class="mySwiper" :modules="modules">
            <SwiperSlide v-for="(slide, index) in necessidadesEspeciais" :key="index">
              <div class="list-item">
                <div class="flex between">
                  <strong>{{ slide.turma }}</strong>
                  <div class="header-badge">
                    {{ slide.tag }}
                  </div>
                </div>
                <div v-for="(aluno, i) in slide.alunos" :key="i" class="flex between">
                  <div>{{ aluno.nome }}</div>
                  <div class="badge">
                    {{ aluno.tag }}
                  </div>
